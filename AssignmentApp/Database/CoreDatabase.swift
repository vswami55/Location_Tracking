...
class CoreDatabase {
  ...
  //1
  static let sharedManager = CoreDatabase()
  ...
  //3
  func saveContext () {
    let context = CoreDatabase.sharedManager.persistentContainer.viewContext
  ...
  /*Insert*/
  func insertRecords(dateTime: String, meters : Int32)->TravelledDb? {
     ...
     */
    let managedContext = CoreDatabase.sharedManager.persistentContainer.viewContext
     ...
     */
    let entity = NSEntityDescription.entity(forEntityName: "TravelledDb",
                                            in: managedContext)!
     ...
     */
    let dbData = NSManagedObject(entity: entity,
     ...
     */
      dbData.setValue(dateTime, forKeyPath: "dateTime")
      dbData.setValue(meters, forKeyPath: "meters")
  ...
  func update(dateTime:String, meters : Int32, localData : TravelledDb) {
    ...
    do {
      ...
      /*
       With an NSManagedObject in hand, you set the name attribute using key-value coding. You must spell the KVC key (name in this case) exactly as it appears in your Data Model
       */
        localData.setValue(dateTime, forKey: "dateTime")
        localData.setValue(meters, forKey: "meters")
      
      print("\(localData.value(forKey: "dateTime"))")
      print("\(localData.value(forKey: "meters"))")
  ...
  /*delete*/
  func delete(localData : TravelledDb){
    
    let managedContext = CoreDatabase.sharedManager.persistentContainer.viewContext
    
    do {
      
      managedContext.delete(localData)
  ...
  func fetchAllRecords() -> [TravelledDb]?{
     ...
     */
    let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "TravelledDb")
    
    /*You hand the fetch request over to the managed object context to do the heavy lifting. fetch(_:) returns an array of managed objects meeting the criteria specified by the fetch request.*/
    do {
      let records = try managedContext.fetch(fetchRequest)
      return records as? [TravelledDb]
    } catch let error as NSError {
  ...
  func delete(meters: String) -> [TravelledDb]? {
    ...
    do {
      ...
      for i in item {
        ...
        arrRemovedPeople.append(i as! TravelledDb)
      }
